...
export default function AddLesson({ location, history }) {
    ...
    return (
        <Card title={<PageHeader className="add-lesson-header" onBack={onBack} title="新增课时"/>}>
            <Form {...layout} onFinish={onFinish} initialValues={{free:true}}>
                <Form.Item label="课时名称" name="title" rules={[{required:true,message:'请输入课时名称'}]}>
                    <Input/>
                </Form.Item>
                <Form.Item label="是否免费" name="free"  rules={[{required:true,message:'请选择是否免费'}]} valuePropName="checked">
